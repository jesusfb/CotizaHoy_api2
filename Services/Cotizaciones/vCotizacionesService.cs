...
{
    public class vCotizacionesService : IvCotizacionesService
    {
        private readonly OurHeroDbContext _db;

        public vCotizacionesService(OurHeroDbContext db)
        {
            _db = db;
        }
        public async Task<List<vCotizaciones>> GetAll()
        {
         
            return await _db.vCotizaciones.ToListAsync();
        }
